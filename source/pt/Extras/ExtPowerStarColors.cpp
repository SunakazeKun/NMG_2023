...
namespace pt {
	...
	*/
	...
	void getPowerStarIcon(wchar_t* pStr, u32 fontCode) {
		const char* stageName;
		s32 scenarioId;

		asm("mr %0, r27" : "=r" (stageName));
		asm("mr %0, r31" : "=r" (scenarioId));

		s32 starColor = getPowerStarColor(stageName, scenarioId);
		MR::addPictureFontCode(pStr, cPowerStarIconCodes[starColor]);
	}

	kmCall(0x80041E30, getPowerStarIcon);
}
